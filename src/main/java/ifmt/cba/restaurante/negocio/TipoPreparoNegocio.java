...
public class TipoPreparoNegocio {
    ...
    public TipoPreparoDTO alterar(TipoPreparoDTO tipoPreparoDTO) throws NotValidDataException, NotFoundException {
        ...
        try {
            if (tipoPreparoRepository.findById(tipoPreparo.getCodigo()).orElse(null) == null) {
    ...
    public void excluir(int codigo) throws NotValidDataException {
        ...
        try {
            TipoPreparo tipoPreparo = tipoPreparoRepository.findById(codigo).orElse(null);
            if (tipoPreparo == null) {
                throw new NotFoundException("Nao existe esse tipo de preparo");
            }
    ...
    public TipoPreparoDTO pesquisaDescricao(String parteDescricao) throws NotFoundException {
        try {
            return this.toDTO(tipoPreparoRepository.findByDescricaoIgnoreCaseStartingWith(parteDescricao));
        } catch (Exception ex) {
    ...
    public TipoPreparoDTO pesquisaCodigo(int codigo) throws NotFoundException {
        try {
            return this.toDTO(tipoPreparoRepository.findById(codigo).orElse(null));
        } catch (Exception ex) {
            throw new NotFoundException("Erro ao pesquisar tipo de preparo pelo codigo - " + ex.getMessage());
        }
    }
